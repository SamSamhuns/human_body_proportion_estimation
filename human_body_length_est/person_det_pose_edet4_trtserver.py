...
def run_pdet_pose(
    ...
    image_data, all_req_imgs_orig, all_req_imgs_orig_size, fps = (
        extract_data_from_media(FLAGS, preprocess_dtype, filenames, w, h)
    )
    ...
    for response in responses:
        ...
        for i, (heatmap, box) in enumerate(zip(heatmaps, boxes)):
            ...
            ig_kp_idx = {
                i
                for i, score in enumerate(keypts_score)
            ...
            if FLAGS.result_save_dir is not None:
                ...
                plot_one_box([x1, y1, x2, y2], drawn_img, color=cmaps[i % 2])

                # uncomment to draw skeletons on orig image
                PoseEstimator.draw_skeleton_from_keypts(
                    drawn_img,
                    keypts,
                    ignored_kp_idx=ig_kp_idx,
                    color=cmaps[i % 2],
                    thickness=crop_width // 150,
                )
                ...
                PoseEstimator.plot_and_save_heatmap(
                    heatmap,
                    f"{FLAGS.result_save_dir}/heatmap_{i}_{str(counter).zfill(6)}.jpg",
                )

        if FLAGS.result_save_dir is not None:
            if FLAGS.inference_mode == "image":
                cv2.imwrite(
                    f"{FLAGS.result_save_dir}/frame_{str(counter).zfill(6)}.jpg",
                    drawn_img,
                )
