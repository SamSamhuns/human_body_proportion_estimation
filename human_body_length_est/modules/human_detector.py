...
class HumanDetectorTflite:
    ...
    def inference(self, frame, thres=0.59):
        ...
        input_img = cv2.resize(
            cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), (self.inwidth, self.inheight)
        )
        ...
        output_class_filter = (output_data_2 == 0.0)[0]  # only use human class
    ...
    def get_people_crops(frame, bboxes):
        ...
        for i in range(bboxes.shape[0]):
            c1, c2 = (
                (int(max(bboxes[i, 1], 0) * w), int(max(bboxes[i, 0], 0) * h)),
                (int(max(bboxes[i, 3], 0) * w), int(max(bboxes[i, 2], 0) * h)),
            )
