...
class PoseEstimator:
    ...
    def preprocess(frame_s, w=288, h=384) -> np.ndarray:
        ...
        if isinstance(frame_s, list):
            frame_s = np.array([cv2.resize(frame, (w, h)) for frame in frame_s])
        if len(frame_s) == 3:
    ...
    def inference(self, frame_s) -> np.ndarray:
        ...
        """
        onnx_rt_inputs = {
            self.input_name: PoseEstimator.preprocess(frame_s, self.w, self.h)
        }
    ...
    def get_max_pred_keypts_from_heatmap(heatmap) -> tuple:
        ...
        maxidx, maxvals = np.argmax(heatmap_reshaped, 1), np.max(heatmap_reshaped, 1)
        maxidx, maxvals = (
            maxidx.reshape((num_joints, 1)),
            maxvals.reshape((num_joints, 1)),
        )
    ...
    def plot_keypts(frame, keypts, color, ignored_kp_idx=None) -> None:
        ...
        for i, (x, y) in enumerate(keypts):
            if i not in ignored_kp_idx_set:
                ...
                cv2.circle(frame, (x, y), frame.shape[0] // 150, color, thickness=-1)

    @staticmethod
    def _get_kp_dict(keypts, ignored_kp_idx) -> Dict:
        """Get a connections name keypoint dict name_kp_dict"""
        ignored_kp_idx = set(ignored_kp_idx) if ignored_kp_idx is not None else {}
        # set to use for skeleton connection, use_set
        uset = {
            val for key, val in IDX_TO_KEYPOINTS.items() if key not in ignored_kp_idx
        }
        ...
        if {"rshoulder", "lshoulder"} <= uset:
            chest = [
                int(rshoulder[0] + lshoulder[0]) // 2,
                int(rshoulder[1] + lshoulder[1]) // 2,
            ]
            uset.add("chest")
        if {"rhip", "lhip"} <= uset:
            crotch = [int(rhip[0] + lhip[0]) // 2, int(rhip[1] + lhip[1]) // 2]
            uset.add("crotch")

        # reverse perspective to that of the image person
        kp_dict = {
            "shoulder": [{"rshoulder", "lshoulder"}, (rshoulder, lshoulder)],
        ...
        name_kp_dict = {k: v[1] if v[0] <= uset else None for k, v in kp_dict.items()}
        return name_kp_dict
    ...
    @staticmethod
    def _draw_line_bet_keypts(
        kp_dict: Dict[str, tuple], frame: np.ndarray, color: tuple, thickness: int
    ) -> None:
        for part_name, part_kp in kp_dict.items():
            if part_kp is not None:  # if required points are not missing
    ...
    @staticmethod
    def get_keypoint_dist_dict(
        pixel_to_cm, keypts, ignored_kp_idx=None
    ) -> Dict[str, float]:
        ...
        dist_dict = {
            k: v * pixel_to_cm if v > 0 else "Part not visible"
            for k, v in name_kp_dict.items()
        }
        return dist_dict

    @staticmethod
    def draw_skeleton_from_keypts(
        frame, keypts, ignored_kp_idx=None, color=(0, 0, 255), thickness=1
    ) -> None:
        """
        args:
            frame: numpy ndarray of shape (H,W,C)
        ...
        PoseEstimator._draw_line_bet_keypts(name_kp_dict, frame, color, thickness)
