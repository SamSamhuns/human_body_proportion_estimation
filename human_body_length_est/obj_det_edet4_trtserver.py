from modules.triton_utils import (
...
def postprocess(results, output_name):
    ...
    # edetlite4
    if (
        "output_0" in output_set
        and "output_0" in output_set
        and "output_0" in output_set
    ):
    ...
    # edetlite4 modified
        ...
        "detection_boxes" in output_set
        and "detection_classes" in output_set
        and "detection_scores" in output_set
    ):
...
def run_demo_odet(
    ...
    image_data, all_reqested_images_orig, all_req_imgs_orig_size, fps = (
    ...
    for response in responses:
        ...
        if FLAGS.result_save_dir is not None:
            ...
            for box_yxyx in det_boxes:
                ...
                plot_one_box(
                    box_xyxy, drawn_img, wscale=wscale, hscale=hscale, color=(255, 0, 0)
                )
            if FLAGS.result_save_dir is not None:
