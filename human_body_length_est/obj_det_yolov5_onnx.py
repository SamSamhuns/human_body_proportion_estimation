...
from modules.onnx_utils import (
    letterbox_image,
    scale_coords,
    non_max_suppression,
    w_non_max_suppression,
)
...
def save_output(
    detections, image_src, save_path, line_thickness=None, text_bg_alpha=0.0
):
...
def detect_onnx(
    ...
    for i, (orig_input, model_input) in enumerate(data_stream):
        batch_size = (
            model_input.shape[0]
            if isinstance(model_batch_size, str)
        ...
        if output_dir is not None:
            save_path = os.path.join(output_dir, f"frame_{str(i).zfill(5)}.jpg")
            save_output(
                batch_detections[0],
                orig_input,
                save_path,
...
if __name__ == "__main__":
    ...
    print(f"Inference time ({onnxruntime.get_device()}): {time.time() - t1:.2f}s")
